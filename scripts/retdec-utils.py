...
class CmdRunner:
    ...
    def _start(cmd, buffer_output=False, discard_stdout=False, discard_stderr=False):
        ...
        if discard_stderr:
            ...
            stderr = subprocess.PIPE if discard_stdout else subprocess.STDOUT
        else:
            stderr = None
        ...
        return _WindowsProcess(**kwargs) if is_windows() else _LinuxProcess(**kwargs)
    ...
    def _get_memory_from_measured_output(output):
        ...
        memory = 0
        if s := re.search(r'Maximum resident set size \(kbytes\): (\d+)', output):
            if g := s[1]:
                memory_kb = int(g)
...
def check_python_version():
    if sys.version_info < (3, 4):
        print_error_and_die(
            f'Cannot use Python version {platform.python_version()} ({sys.executable}). Use at least Python 3.4.'
        )
